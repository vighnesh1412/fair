...
void outputImpossible()
 ...
 void calculate(long int n,long long int m)
  { 
  int p,q,s,round1,rounds,flag=0; float f,sum=0; 
  for(q=2;q<n;q++) 
  { 
  for(p=q-1;p<q;p++) 
{ 
...
break; 
 ...
 if(flag==0) outputImpossible(); 
 }
  void getInput(long int *n,long long int *m) 
  { 
  long int pirates; long long loot;
   ...
   *m=loot; 
   ...
   else outputImpossible(); 
   } 
   void main() { long int n=0; long long int m; 
    getInput(&n,&m); 
	if(n!=0) 
	calculate(n,m);
	 getch(); 
